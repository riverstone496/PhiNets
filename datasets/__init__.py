...
def load_cifar5m(local_dir='../data/cifar5m', train=True, memory=False):
    base_url = 'https://storage.googleapis.com/gresearch/cifar5m/'
    npart = 1000448
    
    print('Loading CIFAR 5mil...')
    if memory:
        local_file = download_file(f'{base_url}part0.npz', os.path.join(local_dir, 'part0.npz'))
...
def get_dataset(dataset, data_dir, transform, train=True, download=False, debug_subset_size=None, memory=False):
    if dataset == 'mnist':
        dataset = torchvision.datasets.MNIST(data_dir, train=train, transform=transform, download=download)
    elif dataset == 'stl10':
        if memory:
            dataset = torchvision.datasets.STL10(data_dir, split='train', transform=transform, download=download)
        else:
        ...
        if memory:
            X, Y = load_cifar5m(train=False, memory=True)
        elif train:
            X, Y = load_cifar5m(train=True, memory=False)
        else:
            X, Y = load_cifar5m(train=False, memory=False)
        dataset = CIFAR5mDataset(X, Y, transform=transform)
